...
fun MP_imp thm1 thm2 =
  ...
  implies_elim (Thm.instantiate ([], [((("P",0), HOLogic.boolT), P), ((("Q",0), HOLogic.boolT), Q)]) Isabelle.mp) thm1
    ...
    val thm1_renamed = 
      if special 
        then implies_elim (implies_intr (mk_cTrueprop P) thm1') (assume (mk_cTrueprop P))
        else thm1'
    *)
  in
      implies_elim
...
fun prim_type_definition_reg (name as {Thy, Tyop}, thm) = let
  ...
  val typedef_thm = Isabelle.prove_typedef_hol4 exI typedef_thm typedef_equiv proof_obligation
in
  typedef_thm
end
...
fun define_or_retrieve_const eq thy =
  ...
  case Isabelle_Definitions.get cname thy of
    SOME thm => if Isabelle.Term.aconv (Thm.prop_of thm, eq) orelse (enable_debug andalso String.isPrefix "bool" cname)
                  then (thm, thy)
                  else raise ERR "define_or_retrieve_const" ("Attempting to redefine constant " ^ cname)
  | NONE =>
    let
