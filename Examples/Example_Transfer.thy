...
lemma hol4_Ex_transfer[transfer_rule]: "((A ===> (=)) ===> (=)) Ex HOL4\<open>(?)\<close>" if "bi_total A"

  unfolding bool_ex_equiv[abs_def]
  supply [transfer_rule] = that
  by (transfer_prover)
...
lemmas hol4_div_clauses = [[hol4_thm \<open>arithmeticTheory.DIVISION\<close>]]
end
...
lemma h4_leq_transfer[transfer_rule]: "(rh4_nat ===> rh4_nat ===> (=)) (\<le>) HOL4\<open>(<=)\<close>"
  ...
  by transfer_prover
lemma h4_gr_transfer[transfer_rule]: "(rh4_nat ===> rh4_nat ===> (=)) (>) HOL4\<open>(>)\<close>"
  unfolding hol4_gr_def
  by transfer_prover
lemma h4_geq_transfer[transfer_rule]: "(rh4_nat ===> rh4_nat ===> (=)) (\<ge>) HOL4\<open>(>=)\<close>"
  unfolding hol4_geq_def Orderings.order_class.dual_order.order_iff_strict
  by transfer_prover
...
lift_definition h4_list_all2::"('a \<Rightarrow> 'b \<Rightarrow> bool) \<Rightarrow> 'a list\<E>036list \<Rightarrow> 'b list\<E>036list \<Rightarrow> bool"
  is list_all2 .
...
(* allDistinct *)
lemmas hol4_allDistinct_def = [[hol4_thm \<open>listTheory.ALL_DISTINCT\<close>, untransferred]]
lemma hol4_allDistinct_clauses:
  "list\<E>036ALL_DISTINCT list\<E>036NIL = True"
  "list\<E>036ALL_DISTINCT (list\<E>036CONS h t) = (\<not> bool\<E>036IN h (list\<E>036LIST_TO_SET t) \<and> list\<E>036ALL_DISTINCT t)"
  using hol4_allDistinct_def unfolding bool_all_equiv by metis+
lemma distinct_rec_list: "distinct xs = rec_list True (\<lambda> x xs res. (x \<notin> set xs) \<and> res) xs"
  by (induction xs) auto
lemma allDistinct_rec_hol4_list: "HOL4\<open>ALL_DISTINCT\<close> (xs::'a list\<E>036list) = rec_hol4_list HOL4\<open>T\<close> (\<lambda> x xs res. (\<not> bool\<E>036IN x (list\<E>036LIST_TO_SET xs)) \<and> res)xs"
  unfolding bool_T_equiv by (induction xs rule: hol4_list_induction) (auto simp add: hol4_allDistinct_clauses)

lemma h4_allDistinct_transfer[transfer_rule]:
