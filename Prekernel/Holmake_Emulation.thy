    ...
    val () = Times.get thy
      ...
      |> map_filter (fn (k, t) =>
        if String.isSuffix scriptsml k then
          SOME (XML.Text (String.substring (k, 0, String.size k - String.size scriptsml) ^ " " ^
...
fun export_dir_timings suffix =
  ...
  in map (fn d =>
    XML.Text (String.substring (d, String.size prefix, String.size d - String.size prefix) ^ " " ^
...
fun Holmake_gen1 rbnd forget deps {scripts,all_files,modules,exceptions} dir =
  let
    val time_start = Timing.start ()
    ...
    val modules =
      ...
      (if all_files then map strip_ext files |> distinct op= |> filter_out (fn s => s = "selftest" orelse exists (fn s' => s = s') exceptions) else [])
...
val make_theories = {scripts = true, all_files = false, modules=[], exceptions=[]}
val make_all = {scripts = false, all_files = true, modules=[], exceptions=[]}
fun make_modules modules = {scripts = false, all_files = false, modules=modules, exceptions=[]}
fun make_all_except exceptions = {scripts = false, all_files = true, modules=[], exceptions=exceptions}
