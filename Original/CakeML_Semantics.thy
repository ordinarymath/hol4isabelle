...
[
  "llistTheory"
] ) "../HOL/src/coalgebras"\<close>

ML \<open>

local open HolKernel boolLib bossLib Parse BasicProvers listTheory rich_listTheory llistTheory pred_setTheory optionTheory;
in
...
val IS_PREFIX_FILTER = Q.store_thm("IS_PREFIX_FILTER",
  `!l1 l2. IS_PREFIX l1 l2 \<Rightarrow> IS_PREFIX (FILTER P l1) (FILTER P l2)`,
  Induct >> simp[IS_PREFIX_NIL] >>
  gen_tac >> Cases >> simp[] >> srw_tac[][]);

val less_opt_def = Define `
  (less_opt n NONE \<Leftrightarrow> T) \<and>
  (less_opt n (SOME m) \<Leftrightarrow> n < m)`;

val less_opt_SUC_elim = Q.store_thm("less_opt_SUC_elim",
  `less_opt (SUC n) z \<Rightarrow> less_opt n z`,
  Cases_on`z`>>srw_tac[][less_opt_def]>>simp[])

val less_opt_LLENGTH_LNTH_SOME = Q.store_thm("less_opt_LLENGTH_LNTH_SOME",
  `less_opt n (LLENGTH l) \<Leftrightarrow> IS_SOME (LNTH n l)`,
  EQ_TAC >- (
    Cases_on`LFINITE l` >- (
...
val IS_SOME_LTAKE_less_opt_LLENGTH = Q.store_thm("IS_SOME_LTAKE_less_opt_LLENGTH",
  `IS_SOME (LTAKE n ll) \<Rightarrow> LLENGTH ll = SOME n \<or> less_opt n (LLENGTH ll)`,
  srw_tac[][IS_SOME_EXISTS] >>
...
val LPREFIX_NTH = Q.store_thm ("LPREFIX_NTH",
  `!ll1 ll2.
    LPREFIX ll1 ll2 \<Leftrightarrow> !n. less_opt n (LLENGTH ll1) \<Rightarrow> LNTH n ll1 = LNTH n ll2`,
  ...
  >- (
    ...
    >- (
      ...
      srw_tac[][] >>
      imp_res_tac LFINITE_HAS_LENGTH >>
      `\<forall>x. n \<le> x \<Rightarrow> LNTH x ll2 = NONE` by metis_tac[LNTH_LLENGTH_NONE] >>
      `\<forall>n. n < x \<Rightarrow> LNTH n ll1 \<noteq> NONE` by metis_tac[less_opt_def,less_opt_LLENGTH_LNTH_SOME,optionTheory.IS_SOME_DEF] >>
      `\<not>(n < x)` by metis_tac[arithmeticTheory.LESS_EQ_REFL] >>
...
val lnth_some_length = Q.store_thm ("lnth_some_length",
  `!ll n x. LNTH n ll = SOME x \<Rightarrow> less_opt n (LLENGTH ll)`,
  Induct_on `n` >>
  srw_tac[][] >>
  `ll = [||] \<or> ?h t. ll = h:::t` by metis_tac [llist_CASES] >>
  full_simp_tac(srw_ss())[less_opt_def]
  >- (
    Cases_on `LLENGTH t` >>
    full_simp_tac(srw_ss())[less_opt_def])
...
val llist_shorter_def = Define `
  llist_shorter ll1 ll2 \<Leftrightarrow>
...
`!ll1 ll2.
  ...
  \<Leftrightarrow>
  !n x. LNTH n ll1 = SOME x \<Rightarrow> ?y. LNTH n ll2 = SOME y`,
...
`!l1 l2. llist_shorter (fromList l1) (fromList l2) \<Leftrightarrow> LENGTH l1 \<le> LENGTH l2`,
 srw_tac[][llist_shorter_def] >>
 every_case_tac >>
 full_simp_tac(srw_ss())[LLENGTH_fromList]);

val lprefix_chain_def = Define `
  lprefix_chain ls \<Leftrightarrow>
    !ll1 ll2. ll1 \<in> ls \<and> ll2 \<in> ls \<Rightarrow> LPREFIX ll1 ll2 \<or> LPREFIX ll2 ll1`;

val lprefix_chain_subset = Q.store_thm("lprefix_chain_subset",
  `lprefix_chain ls \<and> y \<subseteq> ls \<Rightarrow> lprefix_chain y`,
  srw_tac[][lprefix_chain_def,SUBSET_DEF]);

val lprefix_chain_LNTHs_agree = Q.store_thm ("lprefix_chain_LNTHs_agree",
  `lprefix_chain ls \<and>
   l1 \<in> ls \<and> l2 \<in> ls \<and>
   LNTH n l1 = SOME x1 \<and>
   LNTH n l2 = SOME x2 \<Rightarrow>
   x1 = x2`,
  srw_tac[][] >>
...
val lprefix_chain_nth_def = Define `
  lprefix_chain_nth n ls =
    some x. ?l. l \<in> ls \<and> LNTH n l = SOME x`;

val exists_lprefix_chain_nth = Q.store_thm ("exists_lprefix_chain_nth",
  `!ls n x.
    lprefix_chain ls \<and>
    (?l. l \<in> ls \<and> LNTH n l = SOME x) \<Rightarrow>
...
val not_exists_lprefix_chain_nth = Q.store_thm ("not_exists_lprefix_chain_nth",
  `!ls n.
    lprefix_chain ls \<and>
    (!l. l \<in> ls \<Rightarrow> LNTH n l = NONE) \<Rightarrow>
...
val lprefix_chain_nth_none_mono = Q.store_thm ("lprefix_chain_nth_none_mono",
 ...
 CCONTR_TAC >>
 full_simp_tac(srw_ss())[] >>
 `LNTH m l \<noteq> NONE` by metis_tac [LNTH_NONE_MONO] >>
 metis_tac [exists_lprefix_chain_nth, NOT_SOME_NONE, option_nchotomy]);

val equiv_lprefix_chain_def = Define `
  equiv_lprefix_chain ls1 ls2 \<Leftrightarrow>
    !n. lprefix_chain_nth n ls1 = lprefix_chain_nth n ls2`;

val equiv_lprefix_chain_thm = Q.store_thm ("equiv_lprefix_chain_thm",
  `!ls1 ls2.
    lprefix_chain ls1 \<and> lprefix_chain ls2
    \<Rightarrow>
    (equiv_lprefix_chain ls1 ls2 \<Leftrightarrow>
      (!ll1 n x. ll1 \<in> ls1 \<and> LNTH n ll1 = SOME x \<Rightarrow> ?ll2. ll2 \<in> ls2 \<and> LNTH n ll2 = SOME x) \<and>
      (!ll2 n x. ll2 \<in> ls2 \<and> LNTH n ll2 = SOME x \<Rightarrow> ?ll1. ll1 \<in> ls1 \<and> LNTH n ll1 = SOME x))`,
  ...
  srw_tac[][] >>
  Cases_on `?s l. l \<in> ls1 \<and> LNTH n l = SOME x` >>
  full_simp_tac(srw_ss())[] >>
  metis_tac [not_exists_lprefix_chain_nth, NOT_SOME_NONE, exists_lprefix_chain_nth, option_nchotomy]);

val equiv_lprefix_chain_thm2 = Q.store_thm ("equiv_lprefix_chain_thm2",
`!ls1 ls2.
  lprefix_chain ls1 \<and> lprefix_chain ls2 \<and> (!ll2. ll2 \<in> ls2 \<Rightarrow> LFINITE ll2) \<Rightarrow>
  (equiv_lprefix_chain ls1 ls2 \<Leftrightarrow>
   (\<forall>ll1 n x.
      ll1 \<in> ls1 \<and> LNTH n ll1 = SOME x \<Rightarrow>
      \<exists>ll2. ll2 \<in> ls2 \<and> LNTH n ll2 = SOME x) \<and>
   (!ll2 n x.
     ll2 \<in> ls2 \<and> ll2 \<noteq> [||] \<Rightarrow> ?ll1. ll1 \<in> ls1 \<and> llist_shorter ll2 ll1))`,
...
val lprefix_lub_def = Define `
  lprefix_lub ls lub \<Leftrightarrow>
    (!ll. ll \<in> ls \<Rightarrow> LPREFIX ll lub) \<and>
    (\<forall>ub. (!ll. ll \<in> ls \<Rightarrow> LPREFIX ll ub) \<Rightarrow> LPREFIX lub ub)`;

val lprefix_lub_is_chain = Q.store_thm ("lprefix_lub_is_chain",
  `!ls ll. lprefix_lub ls ll \<Rightarrow> lprefix_chain ls`,
  srw_tac[][lprefix_lub_def,lprefix_chain_def] >>
  metis_tac[prefixes_lprefix_total]);

(*
val lprefix_lub_nth = Q.store_thm ("lprefix_lub_nth",
  `!ls lub. lprefix_chain ls \<Rightarrow>
    (lprefix_lub ls lub \<Leftrightarrow> !n. LNTH n lub = lprefix_chain_nth n ls)`,
  srw_tac[][lprefix_lub_def,EQ_IMP_THM] >- (
    Cases_on `LNTH n lub` >> srw_tac[][]
    >- (
      `!l. l \<in> ls \<Rightarrow> LNTH n l = NONE` by (
         srw_tac[][] >>
         res_tac >>
    ...
    >- (
      ...
      qpat_assum `\<forall>ub. (\<forall>ll. ll \<in> ls \<Rightarrow> LPREFIX ll ub) \<Rightarrow> LPREFIX lub ub`
  ...
  >- (
    srw_tac[][LPREFIX_NTH] >>
    Cases_on`\<exists>l x. l \<in> ls \<and> LNTH n l = SOME x` >- (
      full_simp_tac(srw_ss())[] >>
...
val build_lprefix_lub_f_def = Define`
  build_lprefix_lub_f ls n =
    OPTION_MAP (\<lambda>x. (n+1, x)) (lprefix_chain_nth n ls)`;
...
val build_lprefix_lub_lem = Q.prove (
  `!ls. lprefix_chain ls \<Rightarrow> !m n. LNTH n (LUNFOLD (build_lprefix_lub_f ls) m) = lprefix_chain_nth (m + n) ls`,
  rpt gen_tac >>
  DISCH_TAC >>
  Induct_on `n` >>
  ...
  full_simp_tac(srw_ss())[]
  >- metis_tac [lprefix_chain_nth_none_mono, DECIDE ``m \<le> m + SUC n``]
  >- simp [arithmeticTheory.ADD1]);

val build_lprefix_lub_thm = Q.store_thm ("build_lprefix_lub_thm",
  `!ls. lprefix_chain ls \<Rightarrow> lprefix_lub ls (build_lprefix_lub ls)`,
...
val lprefix_lub_equiv_chain = Q.store_thm ("lprefix_lub_equiv_chain",
  `!ls1 ls2 ll.
    lprefix_chain ls1 \<and>
    lprefix_chain ls2 \<and>
    equiv_lprefix_chain ls1 ls2
    \<Rightarrow>
    (lprefix_lub ls1 ll \<Leftrightarrow> lprefix_lub ls2 ll)`,
  srw_tac[][] >>
...
val lprefix_lub_equiv_chain2 = Q.store_thm ("lprefix_lub_equiv_chain2",
  `!ls1 ls2 ll1 ll2.
    lprefix_lub ls1 ll1 \<and>
    lprefix_lub ls2 ll2
    \<Rightarrow>
    (ll1 = ll2 \<Leftrightarrow> equiv_lprefix_chain ls1 ls2)`,
  srw_tac[][] >>
  imp_res_tac lprefix_lub_is_chain >>
  eq_tac >>
  srw_tac[][] >>
...
val lprefix_lub_new_chain = Q.store_thm ("lprefix_lub_new_chain",
  `!ls1 ls2 ll.
    lprefix_chain ls2 \<and>
    equiv_lprefix_chain ls1 ls2 \<and>
    lprefix_lub ls1 ll
    \<Rightarrow>
    lprefix_lub ls2 ll`,
...
val prefix_chain_def = Define`
  prefix_chain ls \<Leftrightarrow>
    \<forall>l1 l2. l1 \<in> ls \<and> l2 \<in> ls \<Rightarrow> l1 \<preceq> l2 \<or> l2 \<preceq> l1`;

val prefix_chain_lprefix_chain = Q.store_thm("prefix_chain_lprefix_chain",
  `prefix_chain ls \<Rightarrow> lprefix_chain (IMAGE fromList ls)`,
...
val prefix_chain_FILTER = Q.store_thm("prefix_chain_FILTER",
  `prefix_chain ls \<Rightarrow> prefix_chain (IMAGE (FILTER P) ls)`,
  srw_tac[][prefix_chain_def] >> metis_tac[IS_PREFIX_FILTER])

val build_prefix_lub_intro = Q.store_thm("build_prefix_lub_intro",
  `lprefix_chain ls \<Rightarrow>
   (lprefix_lub ls lub \<Leftrightarrow> (lub = build_lprefix_lub ls))`,
  metis_tac[build_lprefix_lub_thm,unique_lprefix_lub])

Definition lprefix_rel_def:
  lprefix_rel s1 s2 \<Leftrightarrow> \<forall>l1. l1 IN s1 \<Rightarrow> \<exists>l2. l2 IN s2 \<and> LPREFIX l1 l2
End

Theorem lprefix_rel_lnth:
  lprefix_rel ls1 ls2 \<Rightarrow>
  (\<forall>ll1 n x. ll1 \<in> ls1 \<and> LNTH n ll1 = SOME x \<Rightarrow>
             \<exists>ll2. ll2 \<in> ls2 \<and> LNTH n ll2 = SOME x)
Proof
  rw [lprefix_rel_def]
...
Theorem IMP_equiv_lprefix_chain:
  lprefix_chain ls1 \<and> lprefix_chain ls2 \<and>
  lprefix_rel ls1 ls2 \<and> lprefix_rel ls2 ls1 \<Rightarrow>
  equiv_lprefix_chain ls1 ls2
Proof
